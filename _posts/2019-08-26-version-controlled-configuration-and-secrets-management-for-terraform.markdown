...
function fetch_secrets() {
    workspace_name=$1
    module_name=$2
    master_password_for_workspace=$(eval "echo \$MASTER_PASSWORD_$workspace_name")
    echo $(openssl enc -aes-256-cbc -d -in ../config/$workspace_name/$module_name.tfsecrets.enc -pass pass:$master_password_for_workspace | sed '/^$/D' | sed 's/.*/TF_VAR_& /' | tr -d '\n')
}
```
...
if [ -z "$MASTER_PASSWORD" ]; then
    echo "Set a MASTER_PASSWORD for \"$WORKSPACE_NAME\" encryption"
    exit 1
fi
...
function encrypt_file() {
    ...
    openssl enc -aes-256-cbc -salt -in $input_file -out $target_file -pass pass:$MASTER_PASSWORD
    rm -f $input_file
}
...
if [ -z "$MASTER_PASSWORD" ]; then
    echo "Set a MASTER_PASSWORD for \"$WORKSPACE_NAME\" decryption"
    exit 1
fi
...
function decrypt_file() {
    ...
    openssl enc -aes-256-cbc -d -in $input_file -out $target_file -pass pass:$MASTER_PASSWORD
    rm -f $input_file
}
...
for sub_dir in $(find $base_dir -mindepth 1 -maxdepth 1 -type d); do
    workspace_name=${sub_dir#"$base_dir/"}
    password_var_name="\$MASTER_PASSWORD_$workspace_name"
    master_password_for_workspace=$(eval "echo $password_var_name")

    if [ -z "$master_password_for_workspace" ]; then
    ...
    for input_file in config/$workspace_name/*.tfsecrets.enc
    do
        openssl enc -aes-256-cbc -d -in $input_file -pass pass:$master_password_for_workspace &> /dev/null
        if [ $? != 0 ]; then
